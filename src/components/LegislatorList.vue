<template>
    <!-- <div>
        <button @click="selectedRegion = '北部地區'">北部地區</button>
        <button @click="selectedRegion = '中部地區'">中部地區</button>
        <button @click="selectedRegion = '南部地區'">南部地區</button>

        <div v-if="selectedRegion">
            <button v-for="(city, cityName) in electionData[selectedRegion]" :key="cityName"
                @click="showCandidates(cityName)" :class="{ 'active': selectedCity === cityName }">
                {{ cityName }}
            </button>
        </div>
        <div v-if="selectedCity">
            <h2>{{ selectedCity }}</h2>
            <ul>
                <li v-for="(candidate, index) in electionData[selectedRegion][selectedCity]" :key="index">
    ...
    <div class="layout">
        <h2><i class="fa-solid fa-fire fa-beat" style="--fa-animation-duration: 2s;"></i> 立委參選名單</h2>
        <div class=" infogram-embed" data-id="6f7d2b4c-1d4e-489c-b939-286c77b5ee10" data-type="interactive"
            data-title="立委初選（8/7更新）">
        </div>
        <div
            ...
            <a href="https://infogram.com/6f7d2b4c-1d4e-489c-b939-286c77b5ee10"
                style="color:#989898!important;text-decoration:none!important;" target="_blank">立委初選（8/7更新）</a><br><a
