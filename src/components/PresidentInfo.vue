    ...
    <div class="layout-president layout">
        ...
        <ul class="president-navtab pc">
            <li class="pc " :class="{ 'active': index === selectedCandidateIndex }" v-for="(candidate, index) in candidates"
                ...
                <h2>
                    {{ candidate.num }} {{ candidate.party }} {{ candidate.main.name }}/{{ candidate.sub.name }}</h2>
            </li>
        </ul>
        ...
        <select @change="onCandidateSelect" v-model="selectedCandidate" class="mb custom-select form-control info-select ">
            ...
            <option v-for="(candidate, index) in candidates" :key="index" :value="candidate">
                {{ candidate.num }} {{ candidate.party }} {{ candidate.main.name }} / {{ candidate.sub.name }} </option>
        </select>
            ...
            <div class="president-info">
                <div class="up">
                    <div class="president-info-pic">
                        <img loading="lazy"
                            :src="'https://www.ftvnews.com.tw/topics/2024election/images/candidates/' + candidate.main.name + '.png'" />
                    </div>
                </div>
...
export default {
    ...
    created() {
        this.showCandidate(1);
    }, mounted() {
        if (this.candidates.length > 0) {
            this.selectedCandidate = this.candidates[1];
        }
    },
}
